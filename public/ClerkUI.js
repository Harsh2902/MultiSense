...
var ClerkUI = class ClerkUI {
	...
	*/
	constructor(getClerk, getEnvironment, options, moduleManager) {
		...
		if (parsedVersion && !isVersionAtLeast(clerkVersion, MIN_CLERK_JS_VERSION)) incompatibilityMessage = `@clerk/ui@${ClerkUI.version} requires @clerk/clerk-js@>=${MIN_CLERK_JS_VERSION}, but found @clerk/clerk-js@${clerkVersion}. Please upgrade @clerk/clerk-js (or your framework SDK) to a compatible version.`;
	...
	*/
	ensureMounted(opts) {
		return this.#componentRenderer.ensureMounted(opts);
	}
};

//#endregion
export { ClerkUI };
//# sourceMappingURL=ClerkUI.js.map